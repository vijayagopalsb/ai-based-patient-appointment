...
class DataPreprocessor:
    ...
    This class takes a DataFrame of patient data, preprocesses features (categorical,
    numeric, text), prepares a multi-label target, and applies feature selection.
    """
    ...
    def preprocess_data(self, df):
        ...
        This method performs data cleaning, encoding, scaling, TF-IDF transformation,
        and feature selection on the input DataFrame, returning preprocessed features,
        targets, and preprocessing objects.

        Args:
            df (pd.DataFrame): Input DataFrame containing patient data with columns like
                              "Doctors_Required", "Symptoms", "Age", etc.

        Returns:
            tuple: Contains:
                - X (pd.DataFrame): Preprocessed feature matrix with selected features.
                - y (pd.DataFrame): Multi-label target matrix (rows: patients, columns: doctors).
                - scaler (StandardScaler): Fitted scaler for numeric columns.
                - tfidf (TfidfVectorizer): Fitted TF-IDF vectorizer for symptoms.
                - le_dict (dict): Dictionary of fitted LabelEncoders for categorical columns.
                - selector (SelectKBest): Fitted feature selector.
                - all_doctors (list): List of unique doctor specializations.
        """
